...
{ // --- Loop Infinito ---
  if(digitalRead(SD1) == HIGH)
  {
    ...
    if(aguardarComando())
    { // Aguarda o envio do próximo bit por 2s
      if(!digitalRead(SD2))
      {   // Se o segundo bit for 0
      ...
      if(!comando) 
      {
        fecharGarra(ref); // Executa a função identificada
      }
      if(comando)
      {
        abrirGarra(ref);  // com base na referência definida
      }
    }
